...
export default {
  ...
  methods: {
    ...
    getAiList () {
      getResourceInfoFromType('ai').then(({data}) => {
        if (data && data.status === '1') {
          this.aiDataList = data.data
        } else {
          this.aiDataList = []
    ...
    getFixedParams() {
     ...
     // 电销或AI模板
    changeTelTemplate () {
      this.dataForm.resourceCode = this.aiDataList.filter(item => item.id === this.dataForm.resourceId)[0].code
    },
    submitData () {
