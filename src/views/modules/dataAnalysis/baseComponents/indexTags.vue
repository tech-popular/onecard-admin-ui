<template>
  <el-dialog title="标签分组" :modal-append-to-body="false" :append-to-body="true" :visible.sync="visible" width="50%" :close-on-click-modal="false">
    ...
    <div style="display: flex;" v-if="selectedFieldType === '时间'" v-for="(outdata, index) in baseForm.dateDigitalRange" :key="index">
      <el-form :model="outdata" ref="baseForm" :inline="true">
        <!--条件内容区-->
        <div style="display: flex">
          <div class="content-range">区间{{index+1}}</div>
    ...
    <el-form :model="dataForm" ref="dataForm" :rules="dataRules" :inline="true">
      <el-form-item v-if="(baseForm.numberDigitalRange.length === 0 && selectedFieldType === '数值' ) ||  (baseForm.dateDigitalRange.length === 0 && selectedFieldType === '时间')">
...
<script>
import { getDimension, setDimension, getchildrenList, getOperators } from '@/api/dataAnalysis/indexManage'
...
export default {
  ...
  methods: {
    init (val) {
      ...
      getDimension(params).then(({ data }) => {
        if (data && data.status === '1' && data.data) {
          ...
          if (val.fieldType === '数值') {
            this.getOperatorsList('number')
            data.data.indexGroupConfigs.forEach((item, index) => {
              this.baseForm.numberDigitalRange.push({
                params: [
                  {
                    value: item.func === 'between' ? item.dimension.split(',')[0] : '',
                    title: ''
                  },
                  {
                    value: item.func === 'between' ? item.dimension.split(',')[1] : '',
                    title: ''
                  }
                ],
                singleValue: item.func === 'between' ? '' : item.dimension,
                func: item.func,
            ...
            this.getOperatorsList('date')
            data.data.indexGroupConfigs.forEach((item, index) => {
              this.baseForm.dateDigitalRange.push({
                params: [
                  {
                    value: (item.func === 'relative_time' || item.func === 'relative_time_in') ? item.dimension.split(',')[0] : '',
                    title: ''
                  },
                  {
                    value: (item.func === 'relative_time' || item.func === 'relative_time_in') ? item.dimension.split(',')[1] : '',
                    title: ''
                  }
                ],
                datetime: this.isDateSingleShow(item) ? item.dimension : '',
                datetimeRange: item.func === 'between' ? [item.dimension.split(',')[0], item.dimension.split(',')[1]] : [],
                func: item.func,
                startRangeType: item.startRangeType,
                endRangeType: item.endRangeType,
    ...
    // },
    pramasDateBlur (item, val, index) { // 时间 区间的判断
      let params = item.params
      if (params[0].value * 1 >= params[1].value * 1) {
