<template>
  <div class="pane-rules-content">
    ...
    <div style="flex: 1">
      <div v-for="(item, index) in data.childrenRules" :key="index">
        <el-form :model="item" ref="ruleForm" :inline="true" :disabled="from === 'api'">
          <el-form-item
            prop="fieldCode"
          ...
          <div v-if="isEmpty(item)" class="pane-rules-inline">
            <!--string-->
            <div
              v-if="item.fieldType === 'string' || item.fieldType === ''"
              class="pane-rules-inline"
            >
              <!--string型等于或不等于可以输入多个-->
              <el-form-item
              ...
              >
                ...
                >
                  <el-option
                    ...
                    :value="fitem.childrenNum"
                    :key="findex"
                    :label="fitem.childrenValue"
                  />
                </el-select>
              </el-form-item>
            </div>
              ...
              <div v-if="item.func === 'relative_time'" class="pane-rules-inline">
                ...
                <el-form-item prop="subFunc">
                  <el-select v-model="item.subFunc" class="subSelect">
                    <el-option
                      v-for="(fitem, findex) in item.subSelects"
                      :value="fitem.code"
                      :key="findex"
                      :label="fitem.title"
                    />
