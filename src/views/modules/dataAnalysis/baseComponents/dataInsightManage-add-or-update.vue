...
import { savaDataInfo, updateDataInfo, viewDataInfo, importExcelFile, templateDownload, vestPackAvailable, channelsList, custerAvailable } from '@/api/dataAnalysis/dataInsightManage'
    ...
    getDataInfo (id) { // 查看及编辑时请求数据
      viewDataInfo(id).then(({data}) => {
        if (data.status !== '1') {
          ...
          this.$refs.userAttrRule.renderData(data.data.configJson, this.baseForm.channelId)
        }
      })
    },
    renderEnd () {
    ...
    saveHandle (type) {
      ...
      this.$nextTick(() => { // 待页面中的isRequired = true后再执行校验
        ...
        if (!flag) {
          ...
          let params = { ...this.baseForm, ...this.$refs.userAttrRule.lastSubmitRuleConfig, ...this.rejectForm, rejectGroupPackCode: code }
