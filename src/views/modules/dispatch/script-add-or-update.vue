<template>
  ...
  >
    <div slot="title" class="drawer-title">{{canUpdate ? dataForm.id ? '编辑脚本任务' : '新增脚本任务' : '查看脚本任务'}}<i class="el-icon-close drawer-close" @click="drawerClose"></i></div>
    <div class="wrap">
      ...
      <el-form :model="dataForm" :rules="dataRule" ref="dataForm1" label-width="120px" :disabled="!canUpdate">
        ...
        <el-form-item label="Tag标记" prop="tags">
          <el-select v-model="dataForm.tags" placeholder="Tag标记" style="width: 300px" filterable allow-create multiple>
        ...
        <el-form-item label="脚本内容" prop="script">
          <el-input type="textarea" v-model="dataForm.script" placeholder="脚本内容" :rows="13"/>
        </el-form-item>
      </el-form>
      ...
      this.$nextTick(() => {
        ...
        if (id) {
          info(this.id).then(({data}) => {
            ...
            this.dataForm.formDs = data.data.taskType
            this.dataForm.script = data.data.script
