    ...
    <el-dialog title="同步任务配置" :close-on-click-modal="false" :visible.sync="visible">
      <el-form :model="dataForm" ref="dataForm" label-width="120px" :disabled="!canUpdate">
        <el-form-item label="队列名称" prop="queueName">
          <el-input v-model="dataForm.queueName" placeholder="队列名称" />
        </el-form-item>
        <el-form-item label="队列描述" prop="description">
          <el-input v-model="dataForm.description" placeholder="队列描述" />
        </el-form-item>
...
<script>
import { saveOrUpdateTrinoQueue, getTrinoQueueInfo } from '@/api/dispatch/taskManag'
...
export default {
    ...
    methods: {
        init(id) {
            ...
            if (id) {
              console.log('id' + id)
              getTrinoQueueInfo(id).then(({data}) => {
                if (data.code !== 0) {
                  return this.$message.error(data.msg || '获取数据异常')
                }
                this.dataForm.id = data.data.id
                this.dataForm.queueName = data.data.queueName
        ...
        submitData() {
            saveOrUpdateTrinoQueue(this.dataForm).then(({data}) => {
              if (data && data.code === 0) {
                this.$message({
