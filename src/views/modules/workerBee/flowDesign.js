...
function FlowDesigner (
  ...
  var _designer = {}
  this.newNode = {}
  var _jsonNewStep = { key: guid(), text: params.box.text, remark: '', fill: params.box.color }
  /**
   * 初始化图例面板
   * @returns {*}
   */
  ...
  function initToolbar (div) {
    var myPalette =
      G(go.Palette, div, // 必须是DIV元素
        {
          maxSelectionCount: 3,
          // layout: G(go.TreeLayout, { angle: 90, arrangement: go.TreeLayout.ArrangementFixedRoots }),
          nodeTemplateMap: _designer.nodeTemplateMap, // 跟设计图共同一套样式模板
          model: new go.GraphLinksModel([
            { key: guid(), text: params.circle.text, figure: 'Circle', fill: params.circle.color },
            { key: guid(), text: params.sql.text, figure: 'Ellipse', fill: params.sql.color },
            { key: guid(), text: params.box.text, figure: 'RoundedRectangle', fill: params.box.color },
            { key: guid(), text: params.diamond.text, figure: 'Diamond', fill: params.diamond.color },
            { key: guid(), text: params.rectangle.text, figure: 'Rectangle', fill: params.rectangle.color },
            { key: guid(), text: params.end.text, figure: 'Circle', fill: params.end.color }
          ])
        })
    return myPalette
  }
  ...
  function showEditNode (node) {
    console.log((node instanceof go.Node) && node.data.figure)
    ...
    if ((node instanceof go.Node) && node.data.figure === 'Circle') {
      ...
      return
    } else if ((node instanceof go.Node) && node.data.figure === 'RoundedRectangle') {
      clickTouchAction()
    } else if ((node instanceof go.Node) && node.data.figure === 'Ellipse') {
      clickIdear()
    } else if ((node instanceof go.Node) && node.data.figure === 'Diamond') {
      clickJudge()
    } else {
      MessageBox.prompt('编辑', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputValue: node.data.text
      }).then(({ value }) => {
    ...
    if (touchActionName) {
      console.log(touchActionName, 'touchActionName')
      updateNodeData(node, touchActionName)
    }
  }
