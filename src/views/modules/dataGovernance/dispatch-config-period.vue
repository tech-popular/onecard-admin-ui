    ...
    <el-form label-width="110px" :model="dispatchTimeForm" :rules="dataRule" ref="dispatchTimeForm" class="base-form">
        ...
        <el-col :span="16">
          <div>
            <el-form-item label="运行时间：" prop="onceRunTime">
              <el-date-picker v-model="dispatchTimeForm.onceRunTime" type="datetime" value-format="yyyy-MM-dd HH:mm:ss" placeholder="选择日期时间"></el-date-picker>
              <!-- （未指定运行时间，默认立即下发） -->
            </el-form-item>
          </div>
        </el-col>
      ...
      <div class="work-type-pane" v-if="dispatchTimeForm.jobType === 2">
        <el-form-item label="调度周期：" prop="runCycle">
          <el-select v-model="dispatchTimeForm.runCycle" placeholder="请选择" style="width:220px;" @change="disTimeTurnOff(dispatchTimeForm.runCycle)">
            <el-option v-for="item in dispatchCycleList" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="具体时间：" prop="dayOfWeeks" v-if="dispatchTimeForm.runCycle === 'WEEK'">
          <el-select v-model="dispatchTimeForm.dayOfWeeks" multiple collapse-tags placeholder="请选择">
            <el-option v-for="item in dayOfWeeksList" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="具体时间：" prop="dayOfMonths" v-if="dispatchTimeForm.runCycle === 'MONTH'">
          <el-select v-model="dispatchTimeForm.dayOfMonths" multiple collapse-tags placeholder="请选择">
            <el-option v-for="item in dayOfMonthsList" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item
          class="label-remove-margin"
          prop="execTime"
          label-width="30px"
          v-if="dispatchTimeForm.runCycle === 'MONTH' || dispatchTimeForm.runCycle === 'WEEK' || dispatchTimeForm.runCycle === 'DAY'"
        >
          <el-time-picker v-model="dispatchTimeForm.execTime" format="HH:mm:ss" value-format="HH:mm:ss" placeholder="时:分:秒"></el-time-picker>
        </el-form-item>
      ...
      <div class="work-type-pane" v-if="dispatchTimeForm.jobType === 2 && (dispatchTimeForm.runCycle === 'MINUTE' || dispatchTimeForm.runCycle === 'HOUR')">
        <el-form-item label="开始时间：" prop="startTime">
          <el-time-picker v-model="dispatchTimeForm.startTime" :clearable="false" format="HH:mm" value-format="HH:mm" placeholder="时:分"></el-time-picker>
        </el-form-item>
        <el-form-item label="时间间隔：" prop="interval">
          <el-select v-model="dispatchTimeForm.interval" placeholder="请选择">
            <el-option v-for="item in timeIntervalList" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
          <span>{{ disIntervalMess }}</span>
        </el-form-item>
        <el-form-item label="结束时间：" prop="endTime">
          <el-time-picker v-model="dispatchTimeForm.endTime" :clearable="false" format="HH:mm" value-format="HH:mm" placeholder="时:分"></el-time-picker>
        </el-form-item>
      </div>
      <el-form-item
        label="最晚完成时间："
      ...
      >
        <el-time-picker v-model="dispatchTimeForm.latestTime" :clearable="false" format="HH:mm:ss" value-format="HH:mm:ss" placeholder="时:分:秒"></el-time-picker>
      </el-form-item>
