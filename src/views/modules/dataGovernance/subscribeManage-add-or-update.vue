        ...
        <div class="base-pane">
          <div style="width:29%">
            ...
            <el-collapse v-model="activeNames">
              <el-collapse-item title="SQL列表" name="1">
                <div v-for="(item, index) in sqlDataList" :key="index">
                  <span style="font-style: normal;cursor:pointer;margin-right:20px" @click="clickSqlName(item.sqlName, index)">名称：{{item.sqlName}}</span>
                  <span style="float: right">
                    <i class="el-icon-close cursor-pointer" @click="deleteSqlName(item, index)"></i>
                  </span>
                </div>
              </el-collapse-item>
            </el-collapse>
          ...
          <div style="width:60%">
            <el-form-item label="SQL名称" prop="sqlName">
              <el-input v-model="sqlAddData.sqlName"></el-input>
            </el-form-item>
            <el-form-item prop="sqlData" label="SQL：" ref="serviceBeginSqlForm">
              <div style="border:1px solid #dcdfe6; border-radius: 4px; position:relative;">
                <codemirror
                  ref="serviceBeginSql"
                  v-model="sqlAddData.sqlData"
                  :options="cmOptions"
            ...
            <el-form-item style="margin-top:10px;">
              <el-button type="primary" @click="dataSqlSubmit()">测试SQL</el-button>
              <el-button type="primary" @click="SqlAddSubmit()">保存</el-button>
            </el-form-item>
            <div style="margin-left:50px;">
              <el-form-item label="订阅说明" prop="approveReason">
              ...
              <el-form-item label="订阅方式" prop="receiveType">
              ...
              <el-form-item label="订阅方式" prop="接收类型">
                <el-radio v-model="baseForm.receiveType" label="0">链接</el-radio>
                <el-radio v-model="baseForm.receiveType" label="1" style="margin-left:5px;">图片</el-radio>
              </el-form-item>
            </div>
          </div>
        </div>
...
<script>
import { getUsersList, queryEnableList, databaseList } from '@/api/dataGovernance/subscribeManage'
...
export default {
  ...
  methods: {
    ...
    SqlAddSubmit () {
      this.sqlDataList.push(this.sqlAddData)
      this.sqlAddData = {
        sqlName: '',
        sqlData: ''
      }
    ...
    deleteSqlName (index) {
      this.sqlDataList.splice(index, 1)
    },
    clickSqlName (sqlName, index) {
      this.sqlAddData.sqlName = sqlName
      this.sqlAddData.sqlData = this.sqlDataList[index].sqlData
      this.sqlAddData = {
        sqlName: '',
        sqlData: ''
      }
