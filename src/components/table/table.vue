  ...
  <div class=''>
    <el-table :data="list" style="width: 100%" border>
      ...
      <el-table-column label="操作" align="center" :width="operatesWidth" v-if="operates.length">
        <template slot-scope="scope">
          <el-button size="mini" v-for="(btn,index) in operates" v-show="!btn.isShow || btn.isShow && btn.isShow(scope.row)" :type="btn.type" :key="index" @click.native.prevent="btn.method(scope.row)">{{btn.label}}</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
